    ...
    uploadFiles: function() {
      ...
      if (self.fileFilter.length) {
        for(var i = 0, ilen = self.fileFilter.length; i < ilen; i++) {
          var formfile = self.fileFilter[i];
          if (!formfile.successStatus) {
            var file = new FormData();
            // 上传到服务器的字段名称
            file.append("file", formfile);
            ...
            (function(formfile){
              ...
              $.ajax({
                ...
                success: function(data) {
                  ...
                  $(self.container).find('#uploadList_'+formfile.index).addClass('success');
                  $(self.container).find('#uploadList_'+formfile.index).find('.upload-progress').addClass("success");
                  formfile.successStatus = 1;
                  formfile.imgData = data;
                  // 隐藏进度条
                  self.hideProcess();
                  ...
                  self.onSuccess && self.onSuccess(formfile, data);
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                  $(self.container).find('#uploadList_'+formfile.index).removeClass('success');
                  formfile.failStatus = 1;
                  $("#loader_"+self.containerId +formfile.index).removeClass('hidden').addClass('upload-fail');
                  $("#m-layer_"+self.containerId +formfile.index).addClass('none');
                  self.onFailure && self.onFailure(formfile, XMLHttpRequest, textStatus, errorThrown);
                }
              })
